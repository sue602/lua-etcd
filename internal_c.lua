...
function M.recvchunkedbody(readbytes, bodylimit, header, body, callback)
	local result = ""
	local size = 0
...
function M.request(interface, method, host, url, recvheader, header, content, callback)
	...
	if mode == "chunked" then
		body, header = M.recvchunkedbody(read, nil, header, body, callback)
		if not body then
