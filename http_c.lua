...
local internal = require "internal_c"
...
function httpc.request(method, host, url, recvheader, header, content, callback)
	local protocol
	...
	local ok , statuscode, body = pcall(internal.request, interface, method, host, url, recvheader, header, content, callback)
	finish = true
	socket.close(fd)
	if interface.close then
		interface.close()
